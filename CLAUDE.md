...
| Jun 18 2026 | updateItemViewer uses viewer slot (char position), NOT WH.Wow.Item type constants | Viewer slots (internal_slot_id + 1) match WH.Wow.Item values for HEAD-HANDS (1-10) but diverge from BACK onward: BACK = viewer slot 15 (not WH.Wow.Item 16), MAIN_HAND = viewer slot 16 (not WH.Wow.Item 21). INVENTORY_TYPE_TO_VIEWER_SLOT in lib/slots.ts maps Blizzard type strings to correct viewer slots. |
