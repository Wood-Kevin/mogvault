...
| Jun 19 2026 | Tier-token gap closed via appearance-set join table (Phase 5) | BC T4/T5/T6 tier pieces come from NPC token exchange; journal indexes the token, not the piece. Built data/token-appearance-sets.json mapping token item IDs → appearance-set IDs, then Phase 5 in build-source-index.ts resolves each set to slot-matched items (first word of token name → slot types) and attributes them to the token's boss sources. 607 tier pieces added total. BC T6 Mage + BC T4 Priest already indexed from retail journal drops — left as-is. Remaining BC gaps closed: BC T6 Priest (sets 351+352 → Forgotten Conqueror → BT/Hyjal ✓), BC T4 Mage (set 898 → Fallen Hero → Kara/Gruul/Mag ✓), BC T5 Priest bonus set (set 353 → Vanquished Defender → SSC/TK ✓). Accuracy rule held throughout — no set added without confirming class and correct instance. |
| Jun 19 2026 | Step 7 visual polish applied | Atmospheric body gradient + logo text-shadow glow; feature cards in stage landing overlay; radial-gradient stage background with platform glow; ring spinner for loading states; farming list counter redesigned as 3xl bold number; instance headers font-bold with py-3; print output gains MogVault brand mark; item cards use h-10 w-10 icons; slot tabs use ring-1 active state. |

*(Append each new entry as its own `| date | decision | reasoning |` line immediately above this note.)*
